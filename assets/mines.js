...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-9c1d697c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-9c1d697c">'+'<span class="tile-face-9c1d697c tile-front-9c1d697c"></span>'+'<span class="tile-face-9c1d697c tile-back-9c1d697c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-9c1d697c")){return;}
var back=tile.querySelector(".tile-back-9c1d697c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-9c1d697c");if(isMine){tile.classList.add("boom-9c1d697c");}
if(dim){tile.classList.add("dimmed-9c1d697c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-9c1d697c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-9c1d697c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-9c1d697c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-9c1d697c");void panelEl.offsetWidth;panelEl.classList.add("jolt-9c1d697c");}
endRound();}
